...
class dg822:
    ...
    def set_output_arb(self,channel_number,sp_freq,volt,phase,offset,data):
        ...
        self.arb_send(':SOURCE%d:APPL:SEQ'%number)
        self.arb_send(':SOURCE%d:FUNC:SEQ:FILT INSERT'%number)
        ...
        self.arb_send(':SOUR%d:VOLT:UNIT VPP'%number)
        self.arb_send(':SOURCE%d:FUNC:SEQ:SRAT %d.000000'%(number,sp_freq))
        self.arb_send(':SOURCE%d:VOLT %.3f'%(number,volt))
        self.arb_send(':SOURCE%d:PHASE %.2f'%(number,phase))
        self.arb_send(':OUTPUT%d:LOAD 50'%number)
    ...
    def set_test_sin(self,channel_number,high_imp):
        ...
        if (high_imp == 1):
            out_str = ":OUTPUT%d:LOAD INFinity"%number
        else:
            out_str = ":OUTPUT%d:LOAD 50"%number
        self.arb_send(out_str)
        
if __name__ == '__main__':
    rigol_dg = dg822()
    rigol_dg.device_addr_str = "USB0::0x1AB1::0x0646::DG8Q273701992::INSTR"
    ...
    if (init_val == 1):
        ...
        rigol_dg.arb_send(':COUPLING1:PHAS OFF')
        rigol_dg.arb_send(':COUPLING2:PHAS OFF')
        rigol_dg.arb_send(':COUPLING1:PHAS:DEV -90')
        rigol_dg.arb_send(':COUPLING1:PHAS ON')
        rigol_dg.arb_send(':COUPLING2:PHAS ON')
