...
class RigolWaveformManager:
    ...
    def prepare_dac_int_waveform(self, voltage_data, center_v=2.5, span_v=2.5):
        """Convert voltage waveform to signed DAC INT values (-32768..32767)."""
        values = []
        for v in voltage_data:
            vv = float(v)
            code = int(((vv - center_v) / span_v) * 32767)
    ...
    def send_waveform_mmem(self, channel, int_waveform, arb_name):
        """Send waveform using MMEM file path then load it to sequence ARB."""
        self.clear_error_queue()
        payload = bytearray()
        for value in int_waveform:
            payload.extend(int(value).to_bytes(2, byteorder='little', signed=True))

        block_header = f"MMEM:TRAC:ARB:DATA:DAC INT:{arb_name},END,#{len(str(len(payload)))}{len(payload)}"
        try:
            original_timeout = self.rigol.timeout
        except Exception:
            original_timeout = None
        ...
        try:
            ...
            self._write_raw(block_header.encode('ascii') + bytes(payload))

            load_cmds = [
                f"SOURCE{channel}:FUNC:SEQ:ARB:LOAD INT:{arb_name}",
                f"SOURCE{channel}:FUNCTION:SEQUENCE:ARB:LOAD INT:{arb_name}",
            ]
            load_ok = False
            ...
            if not load_ok:
                raise RuntimeError(f"Could not load ARB file to CH{channel}: {arb_name}")

            err = self.query_system_error()
            if err and not err.strip().startswith("0"):
                raise RuntimeError(f"MMEM transfer returned error: {err}")
            return
        except Exception as e:
            raise RuntimeError(f"MMEM waveform transfer failed for channel {channel}: {e}")
        finally:
            if original_timeout is not None:
                try:
                    self.rigol.timeout = original_timeout
    ...
    def send_waveform_trace_bin(self, channel, int_waveform):
        """Send waveform using manual-defined TRACE:DATA:DAC16 BIN in chunks."""
        self.clear_error_queue()

        if not int_waveform:
            raise ValueError("Waveform is empty")

        # Manual recommends sending <=20 kbytes per transfer.
        max_payload_bytes = 20000
        points_per_chunk = max(1, max_payload_bytes // 2)
        ...
        try:
            try:
                self.rigol.timeout = max(int(self.rigol.timeout or 0), 25000)
            except Exception:
                pass

            total = len(int_waveform)
            index = 0
            first = True

            while index < total:
                chunk = int_waveform[index:index + points_per_chunk]
                index += len(chunk)
                ...
                header = (
                    f":SOURCE{channel}:TRACE:DATA:DAC16 BIN,{flag},"
                    f"#{len(str(len(payload)))}{len(payload)}"
                )
                self._write_raw(header.encode('ascii') + bytes(payload))
                first = False
                time.sleep(0.03)

            err = self.query_system_error()
            if err and not err.strip().startswith("0"):
                raise RuntimeError(f"TRACE BIN transfer returned error: {err}")
            return
        except Exception as e:
            raise RuntimeError(f"TRACE BIN transfer failed for channel {channel}: {e}")
        finally:
            if original_timeout is not None:
                try:
                    self.rigol.timeout = original_timeout
    ...
    def send_waveform_trace_code(self, channel, int_waveform):
        """Send waveform using manual-defined TRACE:DATA:DAC16 CODE chunks."""
        self.clear_error_queue()

        if not int_waveform:
            raise ValueError("Waveform is empty")

        # Keep ASCII payload under manual recommendation (~20KB each write).
        chunk_points = 1500
        ...
        try:
            ...
            while index < total:
                chunk = int_waveform[index:index + chunk_points]
                index += len(chunk)
                ...
                data_text = ",".join(str(int(v)) for v in chunk)
                command = f"SOURCE{channel}:TRACE:DATA:DAC16 CODE,{flag},{data_text}"
                self._write(command)
                first = False
                time.sleep(0.03)
            ...
            err = self.query_system_error()
            if err and not err.strip().startswith("0"):
                raise RuntimeError(f"TRACE CODE transfer returned error: {err}")
            return
        except Exception as e:
            raise RuntimeError(f"TRACE CODE transfer failed for channel {channel}: {e}")
        finally:
            if original_timeout is not None:
                try:
                    self.rigol.timeout = original_timeout
    ...
    def download_arrays_to_rigol(self, can_h_data, can_l_data, sample_rate=1000000):
        ...
        try:
            ...
            try:
                h_int = self.prepare_dac_int_waveform(h_waveform)
                self.send_waveform_trace_bin(1, h_int)
                print("    CH1 downloaded via TRACE BIN.")
            except Exception as e:
                print(f"    TRACE BIN failed for CH1, trying TRACE CODE: {e}")
                try:
                    self.send_waveform_trace_code(1, h_int)
                    print("    CH1 downloaded via TRACE CODE.")
                except Exception as e2:
                    print(f"    TRACE CODE failed for CH1, fallback TRACE VOLTAGE: {e2}")
                    self.send_waveform_voltage(1, h_waveform)

            print("  - Downloading CH2 data...")
            try:
                l_int = self.prepare_dac_int_waveform(l_waveform)
                self.send_waveform_trace_bin(2, l_int)
                print("    CH2 downloaded via TRACE BIN.")
            except Exception as e:
                print(f"    TRACE BIN failed for CH2, trying TRACE CODE: {e}")
                try:
                    self.send_waveform_trace_code(2, l_int)
                    print("    CH2 downloaded via TRACE CODE.")
                except Exception as e2:
                    print(f"    TRACE CODE failed for CH2, fallback TRACE VOLTAGE: {e2}")
    ...
    def activate_downloaded_waveform(self, sample_rate=1000000, amplitude=None, offset=None):
        ...
        for ch in (1, 2):
            # Manual-defined sequence controls plus legacy-compatible aliases.
            for cmd in [
                f"SOURCE{ch}:FUNCTION ARB",
                f"SOURCE{ch}:FUNCTION:SEQUENCE:LIST:FILTER INSERT",
                f"SOURCE{ch}:FUNCTION:SEQUENCE:LIST:SRATE {float(sample_rate)}",
                f"SOURCE{ch}:FUNCTION:SEQUENCE:LIST:APPLY",
                f"SOURCE{ch}:FUNCTION:SEQUENCE ON",
                f"SOURCE{ch}:FUNCTION:SEQUENCE:STATE ON",
                f"SOURCE{ch}:FUNC:SEQ:FILT INSERT",
                f"SOURCE{ch}:FUNC:SEQ:SRAT {float(sample_rate)}",
            ]:
                try:
                    self.send_scpi(cmd)
                except Exception:
        ...
        # Avoid post-activation queries here because some units have unstable
        # readback after waveform downloads. Output enable is handled separately.
    ...
    def ensure_outputs_on(self, timeout=2.0):
        """Attempt several SCPI variants to enable outputs.

        On some setups query/readback is unreliable, so this method treats a
        successful write as success and uses queries only as optional best effort.
        """
        if not self.rigol:
        ...
        for ch in (1, 2):
            ...
            if not write_ok:
                raise RuntimeError(f"Channel {ch} could not be enabled by any output command variant")

            # Best-effort verification only; do not fail if readback is flaky.
    ...
    def start_outputs_aligned(self):
        """Align channel phase reference and enable both outputs.

        This path is intended for cases where waveforms are already prepared
        (for example by UltraStation) and we only need synchronized output start.
        """
        if not self.rigol:
            raise RuntimeError("Not connected to instrument")

        # Best effort: force a known phase reference before enable.
        for cmd in [
            "SOURCE1:PHASE 0",
            "SOURCE2:PHASE 0",
        ...
        # Enable outputs without changing waveform/sequence settings.
        self.ensure_outputs_on()

        # Run phase sync again after outputs are on.
        for cmd in [
            "SOURCE1:PHASE:SYNCHRONIZE",
            "SOURCE1:PHASE:SYNC",
            "SOUR1:PHAS:SYNC",
