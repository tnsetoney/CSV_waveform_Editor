...
class DG822CsvGui(tk.Tk):
    def __init__(self):
        ...
        # Most lab workflows use separate CH1/CH2 single-column voltage CSVs.
    ...
    def _upload_dual_worker(self, dual_path, sample_rate, amplitude, offset):
        self._log_from_worker(f"Uploading dual CSV: {dual_path}")
        buffer = io.StringIO()
        try:
            ...
            if ok:
                self._log_from_worker("Upload completed successfully.")
                self._ui_call(messagebox.showinfo, "Success", "Waveform uploaded to DG822.")
            else:
                self._log_from_worker("Upload failed.")
                self._ui_call(messagebox.showerror, "Upload failed", "Rigol reported an upload failure. Check log.")
        except Exception as exc:
            output = buffer.getvalue().strip()
            if output:
                for line in output.splitlines():
                    self._log_from_worker(line)
            self._log_from_worker(f"Upload failed: {exc}")
            self._ui_call(messagebox.showerror, "Upload failed", str(exc))
        finally:
            self._ui_call(self._end_operation)

    def _upload_split_worker(self, ch1_path, ch2_path, sample_rate, amplitude, offset):
        self._log_from_worker(f"Uploading CH1 CSV: {ch1_path}")
        self._log_from_worker(f"Uploading CH2 CSV: {ch2_path}")
        buffer = io.StringIO()
        try:
    ...
    def _download_dual_worker(self, dual_path, sample_rate):
        self._log_from_worker(f"Downloading dual CSV only (no output): {dual_path}")
        buffer = io.StringIO()
        try:
            with redirect_stdout(buffer):
                can_h, can_l = self.manager.load_waveform_csv_path(dual_path)
            ...
            if output:
                for line in output.splitlines():
                    self._log_from_worker(line)
            if ok:
                self._log_from_worker("Download completed successfully. Outputs remain OFF.")
                self._ui_call(messagebox.showinfo, "Download Success", "CSV data downloaded to DG822 memory. Outputs are OFF.")
            else:
                self._log_from_worker("Download failed.")
                self._ui_call(messagebox.showerror, "Download failed", "Rigol reported a download failure. Check log.")
        except Exception as exc:
            output = buffer.getvalue().strip()
            if output:
                for line in output.splitlines():
                    self._log_from_worker(line)
            self._log_from_worker(f"Download failed: {exc}")
            self._ui_call(messagebox.showerror, "Download failed", str(exc))
        finally:
            self._ui_call(self._end_operation)

    def _download_split_worker(self, ch1_path, ch2_path, sample_rate):
        self._log_from_worker(f"Downloading CH1 CSV only (no output): {ch1_path}")
        self._log_from_worker(f"Downloading CH2 CSV only (no output): {ch2_path}")
        buffer = io.StringIO()
        try:
            can_h = self._parse_single_column_csv(ch1_path)
            can_l = self._parse_single_column_csv(ch2_path)
    ...
    def stop_output(self):
        ...
        if not self._begin_operation("stop output"):
            return
        worker = threading.Thread(target=self._stop_output_worker, daemon=True)
        worker.start()

    def _stop_output_worker(self):
        self._log_from_worker("Stopping output on CH1/CH2...")
        try:
            ok = self.manager.stop_output()
            if ok:
                self._log_from_worker("Outputs disabled on CH1/CH2.")
            else:
                self._log_from_worker("Failed to disable outputs.")
        except Exception as exc:
            self._log_from_worker(f"Stop output error: {exc}")
            self._ui_call(messagebox.showerror, "Stop Output Failed", str(exc))
        finally:
            self._ui_call(self._end_operation)
    ...
    def _start_output_worker(self):
        self._log_from_worker("Starting output on CH1/CH2...")
        try:
            ok = self.manager.start_outputs_aligned()
            if ok:
                self._log_from_worker("Outputs enabled on CH1/CH2 (aligned start).")
                self._ui_call(messagebox.showinfo, "Output Started", "CH1 and CH2 outputs are ON.")
            else:
                self._log_from_worker("Failed to enable outputs.")
                self._ui_call(messagebox.showerror, "Start Output Failed", "Could not enable CH1/CH2 outputs.")
        except Exception as exc:
            self._log_from_worker(f"Start output error: {exc}")
            self._ui_call(messagebox.showerror, "Start Output Failed", str(exc))
        finally:
            self._ui_call(self._end_operation)
    ...
    def _set_channel_output_worker(self, channel, enabled):
        ...
        self._log_from_worker(f"Setting CH{channel} {state}...")
        try:
            ok = self.manager.set_channel_output(channel, enabled)
            if ok:
                self._log_from_worker(f"CH{channel} {state} success.")
            else:
                self._log_from_worker(f"CH{channel} {state} failed.")
                self._ui_call(messagebox.showerror, "Channel Output Failed", f"Failed to set CH{channel} {state}.")
        except Exception as exc:
            self._log_from_worker(f"CH{channel} {state} error: {exc}")
            self._ui_call(messagebox.showerror, "Channel Output Failed", str(exc))
        finally:
            self._ui_call(self._end_operation)
